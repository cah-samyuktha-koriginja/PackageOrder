...
export class MasterServicesService {
  ...
  apiEndpoint: string = localStorage.getItem('url');
  ...
  const url = this.apiEndpoint + "/oauth2/v2/token/jwttoken?grant_type=client_credentials";
  ...
  postPatient(request, token) {
    ...
    return this._http.post(this.apiEndpoint+'/wbd/order/api/v1/compliancePackageOrder/save' , request, HeaderData);
  }
}
